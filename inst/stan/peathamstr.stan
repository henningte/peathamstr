...
data{
  ...
  int which_c2[N2]; // index observations to their fine sections
  real<lower = 0> p1_Mod_cumulative_mass_shape;
  real<lower = 0> p2_Mod_cumulative_mass_shape;
  real<lower = 0> p1_clymo_par;
  real<lower = 0> p2_clymo_par;
  real<lower = 0> p1_clymo_alpha;
  real<lower = 0> p2_clymo_alpha;
...
parameters {
  ...
  // Clymo model
  vector<lower = 0>[K_fine] clymo_par; // peat addition rate at catotelm top
  real<lower=0>  clymo_alpha; // peat decomposition rate at each modeled depth
  real<lower = 0> Mod_cumulative_mass_shape;
...
transformed parameters{
  ...
  // Clymo model
  vector[K_fine+1] c_cumulative_mass; // cumulative mass for discrete increments
  vector[N2] Mod_cumulative_mass; // cumulative mass at sampling depths

  c_cumulative_mass[1] = cumulative_mass0;
  c_cumulative_mass[2:(K_fine + 1)] = cumulative_mass0 + cumulative_sum(clymo_par / clymo_alpha .* (1 - exp(- clymo_alpha * (x * delta_c))) .* exp(- clymo_alpha * c_ages[1:(K_fine)])) / 1000;

  {
    vector[N2] Mod_age2 = c_ages[which_c2] + x[which_c2] .* (depth2 - c_depth_top[which_c2]);
    vector[N2] age2 = x[which_c2] .* (depth2 - c_depth_top[which_c2]);
    Mod_cumulative_mass = c_cumulative_mass[which_c2] + (clymo_par[which_c2] / clymo_alpha) .* (1 - exp(- clymo_alpha * age2)) .* exp(- clymo_alpha * (Mod_age2 - age2)) / 1000;
  }
...
model {
  ...
  // Clymo model
  clymo_par ~ gamma(p1_clymo_par, p2_clymo_par);
  clymo_alpha ~ gamma(p1_clymo_alpha, p2_clymo_alpha);
  Mod_cumulative_mass_shape ~ gamma(p1_Mod_cumulative_mass_shape, p2_Mod_cumulative_mass_shape);
  cumulative_mass ~ gamma(Mod_cumulative_mass_shape, Mod_cumulative_mass_shape ./ Mod_cumulative_mass);
}

generated quantities {

  vector[K_fine] nmp_rep;
  vector[K_fine] nmu_rep;
  vector[K_fine] nmr_rep;
  nmp_rep = (c_cumulative_mass[2:(K_fine + 1)] - c_cumulative_mass[1:K_fine]) ./ (x * delta_c);
  nmu_rep = nmp_rep ./ exp(-clymo_alpha * c_ages[2:(K_fine + 1)]);
  for(k in 1:K_fine) {
    nmr_rep[k] = sum(nmp_rep[k:K_fine] ./ exp(- clymo_alpha * c_ages[k]) - nmp_rep[k:K_fine] ./ exp(- clymo_alpha * c_ages[k + 1]));
  }
