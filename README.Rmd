...
# new:
d1 <- pangaear::pg_data(doi = '10.1594/PANGAEA.928183')
d2 <- pangaear::pg_data(doi = '10.1594/PANGAEA.934722')

d <- 
  dplyr::full_join(
    d1[[1]]$data |>
      dplyr::select(1, 2, 3, 5) |>
      setNames(nm = c("depth_midpoint", "thickness", "bulk_density", "C")) |>
      dplyr::filter(!is.na(bulk_density)) |>
      dplyr::mutate(
        C = C/bulk_density,
        depth_midpoint = round(depth_midpoint * 100, 1), # assumed
    ...
    d2[[1]]$data |>
      ...
      hamstr::calibrate_14C_age(age.14C = "age", age.14C.se = "age_sd", cal_curve = "intcal20") %>%
      dplyr::mutate(
         age.14C.cal.se = 
          purrr::map_dbl(seq_len(nrow(.)), function(i) {
            if(! is.na(age[[i]]) & is.na(age.14C.cal[[i]])) {
              Bchron::BchronCalibrate(age[[i]], ageSds = age_sd[[i]], allowOutside = T)$Date1$ageSds
            } else {
              age.14C.cal.se[[i]]
            }
          }),
        age.14C.cal = 
          purrr::map_dbl(seq_len(nrow(.)), function(i) {
            if(! is.na(age[[i]]) & is.na(age.14C.cal[[i]])) {
              Bchron::BchronCalibrate(age[[i]], ageSds = age_sd[[i]], allowOutside = T)$Date1$ages + (2003 - 1950)
            } else {
              age.14C.cal[[i]]
            }
          })
